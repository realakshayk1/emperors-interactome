...
- **Cross-architecture pilot (Pillar B) = CONDITIONAL GO.** Two distinct tests on 12 pilot dimers, kept separate. (i) Corroboration: Boltz-2 reproduces the conformal verdict — certified ipTM 0.71 vs dropped 0.46 (Mann-Whitney p=0.021), AF-M vs Boltz Spearman 0.80 — SIGNIFICANT, so drops aren't AF-M-specific artifacts. (ii) Divergence-as-feature (the actual Phase-3 gate): |Score-Boltz ipTM| larger for dropped (0.13) than certified (0.03), one-sided p=0.090 at n=12 — DIRECTIONAL, NOT significant. Verdict: develop cross-architecture divergence as a CANDIDATE nonconformity feature, do NOT integrate on this evidence; a powered score-matched test on a non-separable map is required. (Corrected after audit: the earlier framing conflated agreement with the disagreement-predicts-correctness gate.)
