...
| **G-INTEGRITY** | Purity firewall + dependence robustness | ✅ **met** | 27 tests (requires `make reproduce` first) incl. 5 adversarial firewall + no-signal control + 3 LOCO held-out-recovery; BY/e-BH floor computed |
...
referee → second real map → LOCO recovery) in ~1 min. `make test` collects 27 tests (four need `data/interim/`, so run `make reproduce` first). `make verify` rechecks every manuscript number against its artifact and needs no interim data.
