...
blocked on a data-access approval (novelty gate not independently verified); D4 is the one live candidate but its G3
...
|---|---|---|---|---|---|---|
| **D1** Perturb-seq target discovery | ⚠ **novelty UNVERIFIED** — CD4⁺ T-cell Perturb-seq with GWAS integration is a crowded, active area, but I did not complete an independent novelty search for this track (the sub-agent parked before verifying); no specific competing paper is confirmed here | ⏸ blocked — DE-stats live on `genome-scale-tcell-perturb-seq.s3.amazonaws.com` (off-allowlist, **grantable**); probe parked on your approval card | not run (data gated) | CPU-feasible once data granted | good journal *if* a corroborated regulator emerges | **PENDING** (needs approval; novelty not established) |
...
(D2 method-saturated, D3 data-ungrantable). D1 remains pending on a data-access approval, its novelty gate not independently verified.
...
- **D1 — parked, not pursued.** D4 is a clean GO, and D1 was left parked on its data-access approval
  with its novelty gate (G1) never independently verified — the sub-agent stopped before completing a
  novelty search, so no specific competing paper is confirmed. D1 is therefore neither a GO nor a
  KILL; it is unresolved and would need both the data grant and a real novelty search to decide.
